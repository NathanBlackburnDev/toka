...
def eating_plan():
    ...
    if request.method == 'POST':
        ...
        existing_plan = db.queryDB('SELECT * FROM eating_plans WHERE user_id = ? AND breakfast = ? AND lunch = ? AND dinner = ? AND snacks = ? AND liquid = ?', [
            breakfast,
            lunch,
            dinner,
            snacks,
...
def update_eating_plan(plan_id):
    ...
    if request.method == 'POST':
        ...
        pattern = r'^[A-Za-z0-9\s]+$'
        snacks_pattern = r'^[A-Za-z0-9\s\u2022]'
        existing_plan = db.queryDB('SELECT * FROM eating_plans WHERE user_id = ? AND breakfast = ? AND lunch = ? AND dinner = ?', [
            breakfast,
            lunch,
            dinner
        ])
